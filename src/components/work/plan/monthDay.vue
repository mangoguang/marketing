<template>
  <div class="oneMonth">
    ...
    <weekHeader />
    <ul class="weekDay">
      <li :class="compareDate(`${year}-${month<10?`0${month}`:month}-${item-BeginDay}`)?'hasPlan':''"
          v-for="(item,index) in 42"
          :key="index">
        <span :class="`${year}-${month<10?`0${month}`:month}-${item-BeginDay}`===selectDate?'active':''"
