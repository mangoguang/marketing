<template>
  <div ref="perDailyList"
       class="perDailyList"
       :style="{marginTop:_localAjax().typename!=='Store Manager'?'19.12vw':'31.619vw'}">
    <div class="timeBox"
         :style="{top:`${top}vw`}">
...
export default {
  ...
  methods: {
    ...
    getDaily(obj) {
      ...
      obj.dailyOk
        ? this.go(obj.userId, obj.userName, obj.createTime)
        : mango.tip('未提交日报')
    },
    go(id, name, date) {
      ...
      this.$router.push({
        ...
        params: { id: id, username: name, date: time }
      })
    },
    isIphoneX() {
    ...
    listen() {
      this.$refs.perDailyList.addEventListener(
        'scroll',
        this.handleScroll,
        true
      )
