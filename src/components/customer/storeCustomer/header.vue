    ...
    <div class="tabBox">
      <ul class="tab">
        <li v-for="(item,index) in headerStatus"
            ...
            :key="`headerStatus${index}`"
            @click="changeStatus(index,'headerStatus')">
          <span>{{item.name}}</span>
      ...
      <ul class="subTab"
        ...
        <li v-for="(item,sindex) in subHeaderStatus"
            @click="changeStatus(sindex,'subHeaderStatus')"
            :key="`subHeaderStatus${sindex}`"
            :class="headerStatus[0].status&&item.status?'on':''">
          <span>{{item.name}}</span>
        </li>
      </ul>
      <div class="lastNum"
           v-show="headerStatus[1].status"
           :style="{bottom:bottom}">
        共{{storeNum}}名
      </div>
      <div v-show="headerStatus[0].status">
        <ul class="filter"
            v-if="subHeaderStatus[0].status">
        ...
        <ul class="filter"
          ...
          <li v-if="!subHeaderStatus[3].status">
            <span @click="showRightTimeSelect"
                  class="last">筛选</span>
          </li>
        </ul>
        ...
        <div :class="showFilter&&subHeaderStatus[0].status?`filterList show`:'filterList'"
          ...
          <ul v-show="subHeaderStatus[0].status">
            <li v-for="(item,index) in filterList"
                :key="index"
                @click="closeFilter(item.val)">
              <label :class="item.val===cusomerAjaxParams.sort?'on':''">
                {{item.name}}
                <input type="radio"
                       :value="item.val"
                       :checked="item.val===cusomerAjaxParams.sort"
                       v-model="cusomerAjaxParams.sort">
                <img src="../../../assets/imgs/selected.png"
                     alt=""
                     v-show="item.val===cusomerAjaxParams.sort">
              </label>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('storeCustomer', [
      ...
      'initCustomerList',
      'setStoreCustomerAjaxParams',
      'setSort'
    ]),
    ...mapMutations('storeApproved', [
      'setApprovedNum',
      'setApprovedParams',
      'setApprovedList',
      'setApprovedScroll',
      'setApprovedPage',
      'setApprovedAllLoaded',
      'initApprovedList'
    ]),
    ...mapMutations('storeClosed', [
      'setClosedNum',
      'setClosedParams',
      'setClosedList',
      'setClosedAllLoaded',
    ...
    ...mapMutations('allCustomer', [
      'setAllCustomerNum',
      'setAllCustomerParams',
      'setAllCustomerList',
      'setAllCustomerAllLoaded',
      'setAllCustomerScroll',
      'setAllCustomerPage',
      'initAllCustomerList'
    ]),
    isIphone() {
    ...
    changeStatus(i, str) {
      let array =
        str === 'headerStatus' ? this.headerStatus : this.subHeaderStatus
...
.header {
  ...
  .top {
    ...
    @include input-style(3.466vw, rgba(255, 255, 255, 0.8));
    span {
      font-size: 8vw;
      color: #fff;
      font-weight: bold;
  ...
  .tabBox {
    ...
    .filter {
      ...
      .filterli {
        ...
        background: url('../../../assets/imgs/pullDown.png') no-repeat right
          center;
      ...
      li:last-child {
        span.last {
          padding-left: 4.266vw;
          padding-right: 4vw;
          background: url('../../../assets/imgs/filter.png') no-repeat right
            center;
