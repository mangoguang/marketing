...
import { baseUrl, Request } from "../../Request/request";
...
export default {
  ...
  methods: {
        ...
        case 1: //区域选择
          ...
          if (this.areaShow) {
            ...
            if (this.ajaxData.typename == "Store Manager") {
              ...
              this.cityList.forEach((value, index) => {
                this.selList.push({
                  name: value.empowerCity,
                  index: index,
                  level:value.cityLevel
                });
              });
            }
          }
    ...
    // 获取门店区域列表
    async getList() {
      var url = "/v1/app/report/getCityLevel";
      if (this.ajaxData.typename == "Store Manager") {
        //店长账号
        ...
        let { status, data } = res;
        if (status) {
          this.areaName = data[this.areaIndex].shopName;
          this.areaMsg = data[this.areaIndex].shopId;
          ...
          // this.setArea({value:{name:this.areaName},index:0})
        }
      } else {
        //经销商账号
        let res = await mango.getAjax(url,{tenantId:this.ajaxData.tenantId});
        // console.log(res);
        let { status, data } = res;
        if (status) {
          this.areaName = data[this.areaIndex].empowerCity;
          this.areaMsg = data[this.areaIndex].empowerCity;
          this.cityList = data;
          this.cityLevel = data[this.areaIndex].cityLevel
          // localStorage.setItem("areaMsg", this.areaMsg);
        }
      }
      var obj = {
    ...
    async getBrand() {
      ...
      if (status) {
        this.brandList = data;
        this.brand = data[this.brandIndex].brand;
