...
chartsInit = (_this, chartName, vertical, salesVal, title, routeTo, chartsIndex, radio, source) => {
  ...
  if (dom) {
    if (series) {
      ...
      if (source) {
        dom.style.height = `${vertical === 'horizontal' ? (series[0].data.length * (window.innerWidth/100) * 10) + 100/(window.innerWidth/100) : 100}vw`
      } else {
