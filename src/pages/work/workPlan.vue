<template>
  <div class="workPlan"
    ...
    <banner title='工作计划'
      ...
      <button type="button"
              class="addPlan"
              @click="newPlan"></button>
    </banner>
    <div class="planContent"
         v-show="!showList">
      <monthDay @getMonthDate="getMonthDate"
                :dateList="dateList"
      ...
      <div class="btn-box">
        <button type="button"
                class="switch"
                ref="switch"
                @click="switchList"><span>切换至列表</span></button>
      </div>

    </div>
    <div class="planContent"
         v-show="showList">
      <planList :dateList="dateList"
                :month="month"
                :year="year"
                :day="day" />
      <div class="btn-box"><button type="button"
                class="switchCalendar"
                ref="switchCalendar"
                @click="switchList"><span>切换至日历</span></button></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name === 'work') {
      ...
      next(vm => {
        vm.getPlanList({
          startDate: `${vm.year}-${
            vm.month < 10 ? `0${vm.month}` : vm.month
          }-01`,
  ...
  computed: {},
  mounted() {
    this.$refs.switch.addEventListener(
      'touchstart',
      function() {
    ...
    this.$refs.switchCalendar.addEventListener(
      'touchstart',
      function() {
