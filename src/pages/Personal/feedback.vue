<template>
  <div class="feedback">
    ...
    <upload ref="upload"
            ...
            :clear="isClear" />
    <div class="form">
      <yan-input v-bind="contactObj"
                 v-model="phone"
                 :showIcon="false" />
    </div>
    <btn text="提交"
         @click.native="submit" />
    <message-box :type="messageBox.type"
                 :btnNum='messageBox.btnNum'
                 v-if="messageBox.showMessageBox">
      {{messageBox.tip}}
...
export default {
  ...
  computed: {
    ...
    // remark: {
    //   get() {
    //     console.log(this.$store.state.feedback.remark)
    //     return this.$store.state.feedback.remark
    //   },
    //   set(val) {
    //     val = val.replace(/[^\w\d\s,./()?，。、！（）]/g, '')
