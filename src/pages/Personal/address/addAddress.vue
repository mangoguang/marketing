<template>
  <div class="addAdress">
    ...
    <ul class="list">
      <li>
        <area-select v-bind="formInfo.areaInfo"
                     v-model="area"
                     readonly
                     @update="updateArea"
                     :showIcon="selectIcon"
      ...
      <li>
        ...
        <yan-one-input v-bind="formInfo.addressInfo"
      ...
      <li>
        <house-select v-bind="formInfo.apartmentType"
                      v-model="apartmentType"
                      @update="updateApartmentType"
                      :showIcon="selectIcon"
                      class="li_border" />
      </li>
      <li>
        <elevator-select v-bind="formInfo.elevatorInfo"
                         v-model="elevator"
                         @update="updateElevator"
                         :showIcon="selectIcon" />
      </li>
    </ul>
    <yan-textarea v-bind="formInfo.remarkInfo"
...
export default {
  ...
  computed: {
    ...mapState('addAddress', ['title', 'formInfo'])
  },
  created() {
      ...
      indexModel
        ...
        .then(res => {
          if (res.code === 0) {
            ...
            this.area = `${res.data.provinceName}${res.data.cityName}${
