<template>
  <div class="addAdress">
    ...
    <ul class="list">
      <li>
        <intention-select v-bind="formInfo.intention"
                          :value="goodsValue"
                          :id="customerId"
                          :url="path"
                          :showIcon="selectIcon" />
      ...
      <li>
        <store-select v-bind="formInfo.store"
                      :value="shopName" />
      </li>
      <li>
        <date-select day='end'
                     v-bind="formInfo.time"
      ...
      <li>
        <source-select v-bind="formInfo.source"
                       :value="sourceName"
                       @update="updateSource"
                       :showIcon="selectIcon" />
      </li>
      <li>
        <date-select v-bind="formInfo.deliver"
                     :value="form.deliverDate"
                     @update="updateDeliver"
                     :showIcon="selectIcon" />
      </li>
    </ul>
    <ul class="list">
      <li>
        <address-select v-bind="formInfo.address"
                        :value="address"
                        :id="customerId"
                        :url="path"
                        :showIcon="selectIcon" />
      ...
      <li>
        <yan-input v-bind="formInfo.house"
                   :value="apartmentType"
                   :readonly='true'
                   @click="listen" />
      </li>
      <li>
        <yan-input v-bind="formInfo.elevator"
                   :value="elevator"
                   :readonly='true'
                   @click="listen" />
      </li>
      <li>
        <reason-select v-bind="formInfo.reason"
                       :value="buyReasonName"
                       @update="updateReason"
                       :showIcon="selectIcon" />
      </li>
      <li>
        <style-select v-bind="formInfo.style"
                      :value="stylePrefName"
                      @update="updateStyle"
                      :showIcon="selectIcon" />
      </li>
      <li>
        <progress-select v-bind="formInfo.progress"
                         :value="progressName"
    ...
    <ul class="list">
      <li style="border-top:1px solid transparent;">
        ...
        <yan-one-input v-bind="formInfo.goods"
                       v-model.trim="form.competingGoods"
                       :readonly='readonly'
                       :maxlength='100' />
      </li>
      <li>
        <color-select v-bind="formInfo.color"
                      :value="colorPrefName"
                      @update="updateColor"
                      :showIcon="selectIcon" />
      </li>
      <li>
        <yan-input v-bind="formInfo.budget"
      ...
      <li>
        <yan-input v-bind="formInfo.paid"
                   v-model.trim="form.depositPaid"
                   :readonly='readonly'
                   :maxlength='8' />
      </li>
      <li>
        <discount-select v-bind="formInfo.discount"
                         :value="argreeDiscountTxt"
                         @update="updateDiscount"
                         :showIcon="selectIcon" />
      </li>
    </ul>
    <yan-textarea v-bind="formInfo.remark"
                  :maxlength='200'
                  :readonly='readonly'
                  v-model.trim="form.remark"></yan-textarea>
    <div class="select">
      <classify-select style="margin-bottom:2.666vw"
                       label="意向分类"
                       @update="updateClassify"
                       name="classify"
                       :checked="form.level"
                       :options="formInfo.classify" />
      <classify-select label="是否紧急"
                       @update="updateUrgency"
                       name="urgency"
                       :checked="urgency"
                       :options="formInfo.urgency" />
    </div>
    <div>
    ...
    <p class="last">到底啦</p>
    <yan-layer-prompt v-if="isPrompt"
                      v-model='failReason'
                      @update='layerUpdate'
                      @cancel="layerCancel">
...
export default {
    ...
    $route(to, from) {
      ...
      if (from.name === 'selectAddress') {
        ...
        if (this.$store.state.addressId) {
          ...
          let address = `${obj.province}${obj.city}${obj.district}${
            obj.address
          }`
      ...
      if (from.name === 'intentionProduct') {
        ...
        if (this.$store.state.checkedList.length > 0) {
          ...
          this.form.goodsList = this.$store.state.checkedList.map(
            (item, index) => {
              let obj = {}
  ...
  computed: {
    ...mapState('addIntention', ['title', 'formInfo', 'titleModule']),
  ...
  methods: {
      ...
      indexModel
        ...
        .then(res => {
          if (res.code === 0) {
            ...
            this.shopName =
              res.data.shopId === '' ? '' : this.getShopName(res.data.shopId)
            ...
            this.buyReasonName =
              res.data.buyReasonName == '' ? '' : res.data.buyReasonName
            this.form.buyReason = res.data.buyReason
            this.stylePrefName =
              res.data.stylePrefName == '' ? '' : res.data.stylePrefName
            this.form.stylePref = res.data.stylePref
            this.progressName =
              res.data.progressName == '' ? '' : res.data.progressName
            this.form.progress = res.data.progress
            this.form.competingGoods =
              res.data.competingGoods == '' ? '' : res.data.competingGoods
            this.colorPrefName =
              res.data.colorPrefName == '' ? '' : res.data.colorPrefName
            this.form.colorPref = res.data.colorPref
            this.form.deliverDate =
              res.data.deliverDate == '' ? '' : res.data.deliverDate
            ...
            let address =
              res.data.addressId === ''
                ? ''
                : this.getAddress(res.data.addressId)
            this.address = address
    ...
    valid() {
      ...
      if (
        this.form.competingGoods !== '' &&
        !dutyReg.test(this.form.competingGoods)
      ) {
      ...
      if (this.form.deliverDate !== '') {
        if (
          !mango.compareTimeStamp(this.form.arrivalDate, this.form.deliverDate)
        ) {
      ...
      if (this.valid()) {
        ...
        indexModel.updateOpportunity(form, keys, obj).then(res => {
          if (res.code == 0) {
            ...
            this.$router.replace({
              name: 'intention',
              params: { opportunityId: this.form.oppId }
            })
          } else {
            mango.tip(res.msg)
          }
    ...
    updateParams(obj) {
      ...
      if (obj.goodsList.length > 0) {
        for (let i = 0; i < obj.goodsList.length; i++) {
          this.$set(
            tempObj,
            `opportunity.goodsList[${i}].goodsId`,
            obj.goodsList[i].goodsId
          )
          this.$set(
            tempObj,
            `opportunity.goodsList[${i}].quantity`,
            obj.goodsList[i].quantity
          )
        }
      }
      let temp = {
