<template>
  <div class="customerInfo">
    ...
    <ul class="infoNav">
      <li v-for="(item, index) in customerTabStatus"
          :key="`customerInfoBtn${index}`"
          :class="{active: customerTabStatus[index].status}"
          @click="infoSelect(index)"><span>{{item.name}}</span></li>
    </ul>
    <!-- 客户信息-->
    <div v-show="customerTabStatus[0].status">
      <customer-msg :isedit="edit"
                    :list="list"
                    :editMsg='editMsg'
                    v-if='!editStatus' />
      <div v-else
           style="margin-top:2.666vw;">
        <newDescript :select='this.list.headPortrait? false : true'
                     :list='list'
                     :areaType='true'
                     :type='"descript"' />
        <btn @click.native="saveMsg()"
             :text="'保存资料'"
             class="theBtn"></btn>
      </div>
    </div>
    <!-- 意向信息-->
    <intentionMsg :isedit="edit"
                  v-show="customerTabStatus[1].status"
                  :list='list.opportunityList'
                  :orgId='list.orgId'
                  :phone="phone" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async saveMsg() {
      ...
      if (
        (!this.newCustomerInfo.phone || this.newCustomerInfo.phone === '') &&
        !this.newCustomerInfo.weChat
      ) {
      ...
      if (
        this.newCustomerInfo.phone &&
        this.phone !== this.newCustomerInfo.phone
      ) {
        let ishasPhone
        if (!testPhone) {
          ...
          ishasPhone = await this.checkPhone(
            'phone',
            this.newCustomerInfo.phone
          )
        }
    ...
    saveData() {
      ...
      indexModel
        .updateCustomer(formdata, arr, obj)
        .then(res => {
          if (res.status) {
