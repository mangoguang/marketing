<template>
  <div class="peopleWork">
    ...
    <div class="barBox" v-if="shopShow">
      ...
      <div v-show="!areaPeopleWorkShow" :style="{width: `100vw`}" ref="areaPeopleWorkContainer"></div>
  ...
  methods: {
    ...
    getdata(val) {
      ...
      if (this.ajaxData.typename == "Store Manager") {
        ...
        this.getareaPeopleWorkData(data);
      }
      this.getpeopleWorkData(data);
    },
    // 人效-整体
    getpeopleWorkData(params) {
      mango.loading("open");
      let _this = this;
      mango.getAjax("/v2/app/report/people/work", params).then(res => {
        mango.loading("close");
        if (res) {
          this.key1 = true;
          res = res.data;
    ...
    getareaPeopleWorkData(params) {
      //接口没有
      this.i += 1;
      mango.loading("open");
      let _this = this;
      mango
        .getAjax("/v1/app/dealer/report/people/work/shop", params)
        .then(res => {
          mango.loading("close");
          if (res) {
