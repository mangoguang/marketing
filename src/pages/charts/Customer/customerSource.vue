<template>
  <div class="customerSource">
    ...
    <div class="barBox" v-if="shopShow">
      ...
      <div v-show="!areaCustomerSourceShow" ref="areaCustomerSourceContainer" style="width:100vw"></div>
  ...
  methods: {
    ...
    getdata(val) {
      ...
      if (this.ajaxData.typename == "Store Manager") {
        ...
        this.getareaCustomerSourceData(data);
      }
      this.getcustomerSourceData(data);
    },
    // ajax请求
    getcustomerSourceData(params) {
      mango.loading("open");
      let _this = this;
      mango.getAjax("/v2/app/report/customer/source", params).then(res => {
        mango.loading("close");
        if (res) {
          this.key1 = true;
          // res = res.data
    ...
    getareaCustomerSourceData(params) {
      this.i += 1;
      mango.loading("open");
      let _this = this;
      mango
        .getAjax("/v1/app/dealer/report/customer/source/shop", params)
        .then(res => {
          mango.loading("close");
          if (res) {
